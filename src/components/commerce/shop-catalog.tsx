  ...
  return <main className="mx-auto max-w-7xl px-5 py-8 md:px-8 md:py-20">
    ...
    <div className="mt-5 flex items-center gap-3 border-b border-black/10 pb-4 md:mt-8">
      <button type="button" aria-expanded={filtersOpen} aria-controls="shop-filters" onClick={() => setFiltersOpen((open) => !open)} className="flex items-center gap-2 border border-black/20 px-4 py-2.5 text-[10px] font-semibold uppercase tracking-[.12em] md:hidden">
        <svg aria-hidden="true" viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"><path d="M4 7h16M7 12h10M10 17h4" /></svg> Filters{activeFilterCount > 0 && <span className="flex h-5 min-w-5 items-center justify-center rounded-full bg-black px-1 text-[9px] text-white">{activeFilterCount}</span>}
      </button>
      <label className="flex min-w-0 flex-1 items-center gap-3"><svg aria-hidden="true" viewBox="0 0 24 24" className="h-5 w-5 flex-none" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><circle cx="10.75" cy="10.75" r="6.75" /><path d="m15.75 15.75 4.25 4.25" /></svg><span className="sr-only">Search shop</span><input type="search" value={query} onChange={(event) => { setQuery(event.target.value); setSort("relevance"); }} placeholder="Search products…" className="min-w-0 flex-1 bg-transparent text-sm outline-none placeholder:text-black/35" /></label>
